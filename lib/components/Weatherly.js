...
export default class Weatherly extends Component {
  ...
  getWeather(location) {
    const hourly =
    $.get(`http://api.wunderground.com/api/${key}/hourly10day/q/CO/${location.location}.json`).then((hourlyWeather) => {
      this.state.hourly.push(hourlyWeather);
      this.setState({
        weather: this.state.hourly,
      });
    });
    const tenDay = $.get(`http://api.wunderground.com/api/${key}/forecast10day/q/CO/${location.location}.json`).then((tenDayWeather) => {
